...
def check_json_model(llm_model):
    """Check if a model can handle JSON-formatted responses correctly.
    
    Args:
        llm_model: The language model to test
    """
    json_model = llm_model.bind(response_format={"type": "json_object"})
    output_parser = StrOutputParser()
    
    print("Checking the model's json mode by asking 'What is the capital of France?'")
    prompt = ChatPromptTemplate.from_messages([
        ("system", "You are a helpful AI assistant. Answer the users query accurately."),
        ("human", "Please answer the question in json format\n What is the capital of {country}?")
    ])

    chain = prompt | json_model | output_parser
    response = chain.invoke({'country': 'France'})
    print(f"The model response is: {response}")
